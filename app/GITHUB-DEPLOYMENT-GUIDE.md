...
3.  Choose a name for your repository (e.g., `linguaweave-afarinesh`).
4.  Set it as `Public` or `Private`.
5.  **Important:** **Do not** check the "Initialize this repository with a README" option.
6.  Click the `Create repository` button.

On the next page, GitHub will show you the repository's URL. It will look something like: `https://github.com/YourUsername/linguaweave-afarinesh.git`. Copy this address.
...
5.  **Connect the local repository to the GitHub repository:**
    ...
    ```bash
    git remote add origin https://github.com/YourUsername/linguaweave-afarinesh.git
    ```
    *   Replace `YourUsername` and `linguaweave-afarinesh.git` with your actual values.
