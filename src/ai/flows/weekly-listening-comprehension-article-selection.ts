...
const SelectWeeklyArticleOutputSchema = z.object({
  ...
  articleType: z.enum(['article', 'speech', 'news']).describe('The type of the selected article.'),
});
export type SelectWeeklyArticleOutput = z.infer<typeof SelectWeeklyArticleOutputSchema>;

export async function selectWeeklyArticle(input: SelectWeeklyArticleInput): Promise<SelectWeeklyArticleOutput> {
  return selectWeeklyArticleFlow(input);
}

const prompt = ai.definePrompt({
  name: 'selectWeeklyArticlePrompt',
